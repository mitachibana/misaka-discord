...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('weather')
		.setDescription('Fetches current Nagano City weather'),
	async execute(interaction) {
        const response = await fetch('https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/nagano%20city?unitGroup=metric&key=65RFGH2XZNTJ66GAHZQQFXEC8&contentType=json');
        const nagano = await response.json();
		await interaction.reply({ content: `*Nagano City Weather*
		...
		Sunset: ${nagano?.currentConditions?.sunset}`, ephemeral: true});
	},
};